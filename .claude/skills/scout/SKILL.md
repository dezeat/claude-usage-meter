...
- **Prior art informs, it never overrides invariants.** A pattern can be popular, canonical, and still disqualified because it needs a dependency, a runtime, or a coupling this repo's laws reject (read them — e.g. `CLAUDE.md`, the architecture spec, the decision log). Surface the popular option _and_ the reason it does or doesn't survive the constraints. The field's consensus does not get a vote your architecture didn't grant it.
