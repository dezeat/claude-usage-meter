...
| `⟳`   | resets in… (the 7d reset also spells out its absolute day, `Tu 16.06`)                      |
| `⎇`   | git branch, on the `current` row (dropped outside a repo)                                   |
| `⌂`   | working tree on the `current` row: `root` for the main tree, the name for a linked worktree |
