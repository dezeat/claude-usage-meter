...
import { updateIndex, readIndex, } from "./index-store.js";
...
async function main() {
    const index = await updateIndex(INDEX_PATH, CLAUDE_DIR, DEFAULT_PRICING).catch(async () => {
        return await readIndex(INDEX_PATH);
    });
    const color = process.env["NO_COLOR"] === undefined || process.env["NO_COLOR"] === "";
    process.stdout.write(formatReport(index ?? emptyIndex(), new Date(), color) + "\n");
}
main().catch((err) => {
