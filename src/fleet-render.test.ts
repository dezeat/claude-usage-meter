...
test("the spend cells lead with the live ses cost and burn, then the session's cache% cell", () => {
  ...
  const month = new Date(NOW_MS).toISOString().slice(0, 7);
  // A $2.14 window delta across a 1-hour span makes the burn the delta itself:
  // 2.14/(3.6e6/3.6e6) = $2.14/hr (#101).
  index.activeSpendWindow = { deltaUsd: 2.14, spanMs: 3_600_000 };
  const { spendCells } = renderFleet(
    index,
    EMPTY_INDEX_PATH,
    "opus",
