...
function rosterCell(live: ClassCount[], color: boolean): string {
  return live
    .map(
      (c) =>
        `${paint("●", "green", color)} ${paint(`${c.count} ${c.cls}`, "brightWhite", color)}`,
    )
    .join(" ");
}
...
export function sesCell(
  ...
  if (ratePerHour === undefined) return cell;
...
function renderSpend(
  ...
  // lifetime average.
  const rate =
    index.activeSpendWindow === undefined
      ? undefined
      : liveRate(
          index.activeSpendWindow.deltaUsd,
          index.activeSpendWindow.spanMs,
        );
