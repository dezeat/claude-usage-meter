...
interface RenderOptions {
  ...
  // Presentation toggles (ADR-0007), defaulted to preserve the block/bar look.
  layout?: Layout;
  meters?: Meters;
...
): { fiveHour?: RateWindow; sevenDay?: RateWindow } {
  return {
...
function limitsCells(
  ...
  if (fiveHour) {
    cells.push(
      renderLimit("5h", fiveHour, FIVE_HOUR_SECONDS, now, color, meters),
    );
  }
  if (sevenDay) {
    cells.push(
      renderLimit("7d", sevenDay, SEVEN_DAY_SECONDS, now, color, meters, true),
    );
  }
...
function renderHud(
  ...
  if (index !== null) {
    ...
    const { spend, fleet } = fleetLineSegments(
      index,
      options.indexPath ?? "",
      activeClass(payload),
      payload.costUsd,
  ...
  if (line === "") return paint(payload.modelName ?? "Claude", "dim", color);
  return line;
}
...
export function renderLine(
  ...
  const limits = joinFields(
    limitsCells(payload, options.limits, now, color, meters),
    color,
  );
